"""Render the count2 nested-counting sweep into <root>/seed{S}/eval_{X:02d}/.

Layout matches basic_counting so `evals.aggregate_counting` reads it directly.

Usage:
  python -m evals.render_count2 --out-root /data/videos/count2_sweep \
      --seeds 8 --rig stereo
"""
...
from .count2 import build_scenes, render_scene
...
def main():
    ...
    for seed in range(p.seeds):
        for x in p.counts:
            ...
            print(f"seed{seed} eval_{x:02d}: A={x} B={x+1} C={2*x} cubes", flush=True)
